 ...
 *   - removed a number of include directives since they are already done in flcConsoleGameEngine.h
...
class CaveDiver_SGE : public flc::SDL_GameEngine {
public:
    CaveDiver_SGE()
    {
        sAppName = "CaveDiver_SGE";
    }
...
protected:
    ...
    virtual bool OnUserUpdate( float f )
    {
		...
		if (key_touched( flc::Key::LEFT  )) c -= 40.0f * f;
		if (key_touched( flc::Key::RIGHT )) c += 40.0f * f;
		...
		auto CGE_DrawString = [=]( int x, int y, std::string s, flc::Pixel col ) {
		...
		CGE_DrawString( c - 2, 2, "O-V-O", flc::RED );
		CGE_DrawString( c - 2, 3, " O-O ", flc::RED );
		CGE_DrawString( c - 2, 4, "  V  ", flc::RED );

		// Collision Checking & Health Update
		h -= s[((4 + r) % 28) * 80 + ((int)c - 2)] == '.';

		// Draw HUD
		CGE_DrawString(  0, 27, "===============================================================================", flc::YELLOW );
		CGE_DrawString(  2, 28, "Cave Diver - www.onelonecoder.com - Left Arrow / Right Arrow - Survive!", flc::YELLOW );
		CGE_DrawString(  2, 29, "Distance Fallen: " + std::to_string( (int)(tt * 100.0f)), flc::YELLOW );
		CGE_DrawString( 40, 29, "Health: " + std::to_string( h ), flc::YELLOW );
