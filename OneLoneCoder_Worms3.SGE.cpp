 ...
 * Port to SDL-GameEngine - 20221205
...
public:
	...
	// Make class abstract
	virtual void Draw(flc::SDL_GameEngine *engine, float fOffsetX, float fOffsetY, bool bPixel = false) = 0;
...
public:
	...
	virtual void Draw(flc::SDL_GameEngine *engine, float fOffsetX, float fOffsetY, bool bPixel = false)
	{
		DrawWireFrameModel(engine, vecModel, px - fOffsetX, py - fOffsetY, atan2f(vy, vx), bPixel ? 0.5f : radius, flc::DARK_GREEN);
	}
...
public:
	...
	virtual void Draw(flc::SDL_GameEngine *engine, float fOffsetX, float fOffsetY, bool bPixel = false)
	{
		DrawWireFrameModel(engine, vecModel, px - fOffsetX, py - fOffsetY, atan2f(vy, vx), bPixel ? 0.5f : radius, flc::BLACK);
	}
...
public:
	...
	virtual void Draw(flc::SDL_GameEngine *engine, float fOffsetX, float fOffsetY, bool bPixel = false)
	{
		...
		if (bIsPlayable) // Draw Worm Sprite with health bar, in team colours
		{
			...
			{
				engine->Draw(px - 5 + i - fOffsetX, py + 5 - fOffsetY, flc::BLUE);
				engine->Draw(px - 5 + i - fOffsetX, py + 6 - fOffsetY, flc::BLUE);
			}
		}
...
class OneLoneCoder_Worms : public flc::SDL_GameEngine // The game
{
public:
	OneLoneCoder_Worms()
	{
...
private:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (GetKey(flc::Key::TAB).bReleased)
			bZoomOut = !bZoomOut;
		...
		if (pObjectUnderControl != nullptr)
		{
			...
			{
				if ((bEnablePlayerControl && GetKey(flc::Key::Z).bPressed) || (bEnableComputerControl && bAI_Jump))
				{
				...
				if ((bEnablePlayerControl && GetKey(flc::Key::S).bHeld) || (bEnableComputerControl && bAI_AimRight))
				{
					cWorm* worm = (cWorm*)pObjectUnderControl;
				...
				if ((bEnablePlayerControl && GetKey(flc::Key::A).bHeld) || (bEnableComputerControl && bAI_AimLeft))
				{
					cWorm* worm = (cWorm*)pObjectUnderControl;
					worm->fShootAngle -= 1.0f * fElapsedTime;
					if (worm->fShootAngle < -3.14159f) worm->fShootAngle += 3.14159f * 2.0f;
				...
				if ((bEnablePlayerControl && GetKey(flc::Key::SPACE).bPressed))
				{
					bFireWeapon = false;
					bEnergising = true;
					fEnergyLevel = 0.0f;
				...
				if ((bEnablePlayerControl && GetKey(flc::Key::SPACE).bHeld) || (bEnableComputerControl && bAI_Energise))
				{
					if (bEnergising)
					{
				...
				if ((bEnablePlayerControl && GetKey(flc::Key::SPACE).bReleased))
				{
					if (bEnergising)
					{
						bFireWeapon = true;
		...
		if (!bZoomOut)
		{
			for (int x = 0; x < ScreenWidth(); x++)
				for (int y = 0; y < ScreenHeight(); y++)
				{
                    ...
                    {
                        case -8: Draw( x, y, flc::VERY_DARK_CYAN ); break; // Sky radiants
                        case -7: Draw( x, y, flc::DARK_CYAN      ); break;
                        case -6: Draw( x, y, flc::DARK_CYAN      ); break;
                        case -5: Draw( x, y, flc::BLUE           ); break;
                        case -4: Draw( x, y, flc::DARK_BLUE      ); break;
                        case -3: Draw( x, y, flc::DARK_BLUE      ); break;
                        case -2: Draw( x, y, flc::VERY_DARK_BLUE ); break;
                        case -1: Draw( x, y, flc::VERY_DARK_BLUE ); break;
                        case  0: Draw( x, y, flc::CYAN           ); break; // Simple sky
                        case  1: Draw( x, y, flc::DARK_GREEN     ); break; // Land
                    }
				}
		...
		for (size_t t = 0; t < vecTeams.size(); t++)
		{
            ...
            flc::Pixel cols[] = { flc::RED, flc::BLUE, flc::MAGENTA, flc::GREEN };
		...
		if (bShowCountDown) {
			...
			if (nCountDown < 10) {
                SevenSegmentDisplay( tx, ty, nCountDown, flc::DARK_GREY, 2 );
			}
		}
    ...
    void SevenSegmentDisplay( int x, int y, int digit, flc::Pixel col = flc::WHITE, int scale = 1 ) {
