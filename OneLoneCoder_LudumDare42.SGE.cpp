...
class LudumDare42 : public flc::SDL_GameEngine
{
public:
	LudumDare42()
	{
...
private:
	...
	void GPS_Draw( int x, int y, flc::Pixel colour ) {
        FillRect( x * GAME_PIXEL, y * GAME_PIXEL, GAME_PIXEL, GAME_PIXEL, colour );
	}
	void GPS_DrawString( int x, int y, std::string s, flc::Pixel colour = flc::WHITE ) {
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (IsFocused() && GetKey(flc::Key::SPACE).bReleased)
		{
			if (nBoomsticks > 0)
			{
		...
		for (int x = 0; x < nWorldWidth; x++)
		{
			for (int y = 0; y < nWorldHeight; y++)
			{
				...
				{
				case BORDER:
					GPS_Draw(x, y, flc::WHITE);
					break;

				case BLOCK:
					GPS_Draw(x, y, flc::RED);
					break;
				}
			}
		}
		...
		for (auto &p : listParticles)
		{
			GPS_Draw(p.px, p.py, flc::DARK_YELLOW);
		}

		// Draw Bombs
		for (auto &b : listBombs)
		{
			GPS_Draw(b.px - 1, b.py, flc::GREEN);
			GPS_Draw(b.px + 1, b.py, flc::GREEN);
			GPS_Draw(b.px, b.py - 1, flc::GREEN);
			GPS_Draw(b.px, b.py + 1, flc::GREEN);
		...
		if (bBombsVisible)
		{
			FillRect( (nBombPickupX - 1) * GAME_PIXEL, (nBombPickupY - 1) * GAME_PIXEL, 3 * GAME_PIXEL, 3 * GAME_PIXEL, flc::MAGENTA);
			GPS_Draw(nBombPickupX, nBombPickupY, flc::GREY);
		}
		...
		// Draw Player
		GPS_DrawString(nPlayerX, nPlayerY, "P", flc::CYAN);
