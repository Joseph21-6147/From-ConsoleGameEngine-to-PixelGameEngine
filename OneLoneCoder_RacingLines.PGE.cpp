...
struct sSpline
{
	...
	void DrawSelf(olc::PixelGameEngine* gfx, float ox, float oy, olc::Pixel col = olc::WHITE)
	{
		if (bIsLooped)
		{
...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (GetKey(olc::Key::A).bHeld)	nIterations++;
		if (GetKey(olc::Key::S).bHeld)	nIterations--;
		...
		for (float t = 0.0f; t < path.points.size(); t += fRes)
		{
			...
			FillTriangle(pl1.x, pl1.y, pr1.x, pr1.y, pr2.x, pr2.y, olc::GREY);
			FillTriangle(pl1.x, pl1.y, pl2.x, pl2.y, pr2.x, pr2.y, olc::GREY);
		}
		...
		racingLine.UpdateSplineProperties();
		racingLine.DrawSelf(this, 0, 0, olc::BLUE);

		for (auto i : path.points)
			FillRect(i.x - 1, i.y - 1, 3, 3, olc::RED);

		sPoint2D car_p = racingLine.GetSplinePoint(fMarker);
		sPoint2D car_g = racingLine.GetSplineGradient(fMarker);
		DrawWireFrameModel(this, vecModelCar, car_p.x, car_p.y, atan2f(car_g.y, car_g.x), 4.0f, olc::BLACK);
