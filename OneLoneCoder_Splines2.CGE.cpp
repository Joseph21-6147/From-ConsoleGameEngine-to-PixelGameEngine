...
protected:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		for (int i = 0; i < path.points.size(); i++)
		{
			...
			DrawString(path.points[i].x, path.points[i].y, std::to_wstring(i));
			DrawString(path.points[i].x + 3, path.points[i].y, std::to_wstring(path.points[i].length));
		}
		...
		DrawString(path.points[nSelectedPoint].x, path.points[nSelectedPoint].y, std::to_wstring(nSelectedPoint));
		...
		DrawString(2, 2, std::to_wstring(fOffset));
		DrawString(2, 4, std::to_wstring(fMarker));
