 ...
 * Port to SDL2-GameEngine - 2022-12-04
 *   - standard porting stuff
 */
...
class Tetris_SGE : public flc::SDL_GameEngine {
public:
...
protected:
    ...
    virtual bool OnUserUpdate( float fElapsedTime )
	{
        ...
        auto key_touched = [=]( flc::Key nKey ) {
            return (GetKey( nKey ).bPressed || GetKey( nKey ).bHeld);
        };

		// Handle player movement
        if (key_touched( flc::Key::RIGHT ) && DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX + 1, nCurrentY )) {
                nCurrentX = nCurrentX + 1;
        }
        if (key_touched( flc::Key::LEFT ) && DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX - 1, nCurrentY )) {
                nCurrentX = nCurrentX - 1;
        }
        if (key_touched( flc::Key::DOWN ) && DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX, nCurrentY + 1 )) {
                nCurrentY = nCurrentY + 1;
        }

		// Rotate, but latch to stop wild spinning
		if (key_touched( flc::Key::Z ))
		{
        ...
        auto Char2Pixel = [=]( char c ) -> flc::Pixel {
            switch (c ) {
                case ' ': return flc::BLACK;
                case 'A': return flc::VERY_DARK_GREY;
                case 'B': return flc::RED;
                case 'C': return flc::YELLOW;
                case 'D': return flc::GREEN;
                case 'E': return flc::CYAN;
                case 'F': return flc::BLUE;
                case 'G': return flc::DARK_YELLOW;
                case '=': return flc::VERY_DARK_MAGENTA;
                case '#': return flc::WHITE;
            }
            return flc::MAGENTA;
		};
...
//            DrawString( x * 8, y * 8, std::string( 1, c ));
            FillRect( x * 8 + 1, y * 8 + 1, 8 - 1, 8 - 1, Char2Pixel( c ));
		};
