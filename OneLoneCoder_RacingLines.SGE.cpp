 ...
 *   - added DrawWireFrameModel() function - this function was implemented in the CGE, but not in the PGE
 */
...
struct sSpline
{
	...
	void DrawSelf(flc::SDL_GameEngine* gfx, float ox, float oy, flc::Pixel col = flc::WHITE)
	{
		if (bIsLooped)
		{
...
class OneLoneCoder_RacingLine : public flc::SDL_GameEngine
{
public:
...
protected:
	...
	{
		// Clear Screen
		FillRect(0, 0, ScreenWidth(), ScreenHeight(), flc::DARK_GREEN);

		// Handle iteration count
		if (GetKey(flc::Key::A).bHeld)	nIterations++;
		if (GetKey(flc::Key::S).bHeld)	nIterations--;
		...
		for (float t = 0.0f; t < path.points.size(); t += fRes)
		{
			...
			FillTriangle(pl1.x, pl1.y, pr1.x, pr1.y, pr2.x, pr2.y, flc::GREY);
			FillTriangle(pl1.x, pl1.y, pl2.x, pl2.y, pr2.x, pr2.y, flc::GREY);
		}
		...
		racingLine.UpdateSplineProperties();
		racingLine.DrawSelf(this, 0, 0, flc::BLUE);

		for (auto i : path.points)
			FillRect(i.x - 1, i.y - 1, 3, 3, flc::RED);

		sPoint2D car_p = racingLine.GetSplinePoint(fMarker);
		sPoint2D car_g = racingLine.GetSplineGradient(fMarker);
		DrawWireFrameModel(this, vecModelCar, car_p.x, car_p.y, atan2f(car_g.y, car_g.x), 4.0f, flc::BLACK);
