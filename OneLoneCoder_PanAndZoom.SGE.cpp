 ...
 *   - no changes made in the CGE version
...
class OneLoneCoder_PanAndZoom : public flc::SDL_GameEngine
{
public:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		for (float y = 0.0f; y <= 10.0f; y++)
		{
			if (y >= fWorldTop && y <= fWorldBottom)
			{
				...
				DrawLine(pixel_sx, pixel_sy, pixel_ex, pixel_ey, flc::WHITE);
				nLinesDrawn++;
			}
		}
		...
		for (float x = fWorldLeft; x < fWorldRight; x+=fWorldPerScreenWidthPixel)
		{
			...
			DrawLine(opx, opy, px, py, flc::GREEN);
			opx = px;
			opy = py;
		}
