...
class OneLoneCoder_PerlinNoiseDemo : public flc::SDL_GameEngine
{
public:
...
private:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (GetKey(flc::Key::K1).bReleased)
			nMode = 1;

		if (GetKey(flc::Key::K2).bReleased)
			nMode = 2;

		if (GetKey(flc::Key::K3).bReleased)
			nMode = 3;

		if (GetKey(flc::Key::Q).bReleased)
			fScalingBias += 0.2f;

		if (GetKey(flc::Key::A).bReleased)
			fScalingBias -= 0.2f;
		...
		if (nMode == 1) // 1D Noise
		{
			if (GetKey(flc::Key::Z).bReleased) // Noise Between 0 and +1
				for (int i = 0; i < nOutputSize; i++) fNoiseSeed1D[i] = (float)rand() / (float)RAND_MAX;

			if (GetKey(flc::Key::X).bReleased) // Noise Between -1 and +1
		...
		if (nMode == 2) // 2D Noise
		{
			...
			for (int x = 0; x < nOutputWidth; x++)
			{
				for (int y = 0; y < nOutputHeight; y++)
				{
                    ...
                    col = flc::Pixel( pixel_bw, pixel_bw, pixel_bw );
					Draw( x, y, col );
				}
			}
		}
		...
		if (nMode == 3) // 2D Noise - colourised
		{
			...
			for (int x = 0; x < nOutputWidth; x++)
			{
				for (int y = 0; y < nOutputHeight; y++)
				{
					...
					switch (pixel_bw)
					{
                        case  0: col = flc::BLACK;             break;
                        case  1: col = flc::VERY_DARK_BLUE;    break;
                        case  2: col = flc::DARK_BLUE;         break;
                        case  3: col = flc::BLUE;              break;
                        case  4: col = flc::VERY_DARK_GREEN;   break;
                        case  5: col = flc::DARK_GREEN;        break;
                        case  6: col = flc::GREEN;             break;
                        case  7: col = flc::VERY_DARK_YELLOW;  break;
                        case  8: col = flc::DARK_YELLOW;       break;
                        case  9: col = flc::GREY;              break;
                        case 10: col = flc::WHITE;             break;
					}
