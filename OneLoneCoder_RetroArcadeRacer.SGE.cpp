...
class OneLoneCoder_FormulaOLC : public flc::SDL_GameEngine
{
public:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++)
				Draw(x, y, y< ScreenHeight() / 4 ? flc::VERY_DARK_BLUE : flc::DARK_BLUE);
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++)
			{
				...
				flc::Pixel nGrassColour = sinf(20.0f *  powf(1.0f - fPerspective,3) + fDistance * 0.1f) > 0.0f ? flc::GREEN : flc::DARK_GREEN;
				flc::Pixel nClipColour = sinf(80.0f *  powf(1.0f - fPerspective, 2) + fDistance) > 0.0f ? flc::RED : flc::WHITE;

				// Start finish straight changes the road colour to inform the player lap is reset
				flc::Pixel nRoadColour = (nTrackSection-1) == 0 ? flc::WHITE : flc::GREY;
		...
		{
		case 0:
			DrawString(nCarPos, ScreenHeight() - 64, "   ||####||   ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 56, "      ##      ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 48, "     ####     ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 40, "     ####     ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 32, "|||  ####  |||", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 24, "|||########|||", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 16, "|||  ####  |||", flc::RED);
			break;
		...
		case +1:
			...
			DrawString(nCarPos, ScreenHeight() - 40, "      ####    ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 32, "///  ####//// ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 24, "//#######///O ", flc::RED);
			DrawString(nCarPos, ScreenHeight() - 16, "/// #### //// ", flc::RED);
			break;
