...
public:
	cWorm(float x = 0.0f, float y = 0.0f) : cPhysicsObject(x, y)
	{
		...
		if (sprWorm == nullptr)
			sprWorm = new olcSprite(L"WormsSprites/worms1.spr");
	}
...
private:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (bShowCountDown)
		{
			...
			int a=std::to_wstring(fTurnTime)[c]-48;if(!(r%6)){DrawStringAlpha(tx,
			ty,std::wstring((d[a]&(1<<(r/2))?L" #####  ":L"        ")),FG_BLACK);
			tx+=8;}else{DrawStringAlpha(tx,ty,std::wstring((d[a]&(1<<(r<6?1:4))?
			L"#     ":L"      ")),FG_BLACK);tx+=6;DrawStringAlpha(tx,ty,std::wstring
