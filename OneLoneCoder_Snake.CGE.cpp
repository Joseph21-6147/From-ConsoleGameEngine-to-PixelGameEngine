...
class Snake_CGE : public olcConsoleGameEngine {
    public:

        Snake_CGE() {
            m_sAppName = L"Snake_CGE";
        }
    private:
...
#define TICK_Y 0.200f
            ...
            if (!bDead && fElapsedTimeCumulated > fLocalTick) {
                ...
                auto key_touched = [=]( wchar_t c ) {
                    return (m_keys[ c ].bHeld || m_keys[ c ].bPressed);
                };
                bKeyRight = key_touched( L'D' ) || key_touched( VK_RIGHT );
                bKeyLeft  = key_touched( L'A' ) || key_touched( VK_LEFT  );
                ...
                {
                    nScore++;
                    while (m_bufScreen[nFoodY * ScreenWidth() + nFoodX].Char.UnicodeChar != L' ') {
                        nFoodX =  rand() %  ScreenWidth();
                        nFoodY = (rand() % (ScreenHeight() - 4)) + 3;
                    }
            ...
            DrawString( 5, 1, L"www.OneLoneCoder.com - S N A K E ! !                SCORE: " + std::to_wstring( nScore ) );

			// Draw Snake Body
			for (auto s : snake)
                Draw( s.x, s.y, bDead ? L'+' : L'0' );
			// Draw Snake Head
            Draw( snake.front().x, snake.front().y, bDead ? L'X' : L'@' );
			// Draw Food
            Draw( nFoodX, nFoodY, L'%' );

            if (bDead) {
                std::wstring sText = L"PRESS <SPACE> TO PLAY AGAIN";
                DrawString( (ScreenWidth() - sText.length()) / 2, ScreenHeight() / 2, sText );
