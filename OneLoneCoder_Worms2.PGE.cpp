...
class OneLoneCoder_Worms : public olc::PixelGameEngine
{
public:
	OneLoneCoder_Worms()
	{
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (bPlayerHasControl)
		{
			if (pObjectUnderControl != nullptr)
			{
				...
				{
					if (GetKey(olc::Key::Z).bPressed) // Jump in direction of cursor
					{
					...
					if (GetKey(olc::Key::A).bHeld) // Rotate cursor CCW
					{
					...
					if (GetKey(olc::Key::S).bHeld) // Rotate cursor CW
					{
						cWorm* worm = (cWorm*)pObjectUnderControl;
						worm->fShootAngle += 1.0f * fElapsedTime;
						if (worm->fShootAngle > 3.14159f) worm->fShootAngle -= 3.14159f * 2.0f;
					...
					if (GetKey(olc::Key::SPACE).bPressed) // Start to charge weapon
					{
						bEnergising = true;
						bFireWeapon = false;
						fEnergyLevel = 0.0f;
					...
					if (GetKey(olc::Key::SPACE).bHeld) // Weapon is charging
					{
						if (bEnergising)
						{
					...
					if (GetKey(olc::Key::SPACE).bReleased) // If it is released before maxing out, Fire!
					{
						if (bEnergising)
						{
							bFireWeapon = true;
		...
		for (int x = 0; x < ScreenWidth(); x++)
			for (int y = 0; y < ScreenHeight(); y++)
			{
				...
				{
				case 0:
					Draw(x, y, olc::CYAN); // Sky
					break;
				case 1:
					Draw(x, y, olc::DARK_GREEN); // Land
					break;
				}
			}
