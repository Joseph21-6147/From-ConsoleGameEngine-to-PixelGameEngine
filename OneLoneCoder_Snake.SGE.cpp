...
class Snake_SGE : public flc::SDL_GameEngine {
    public:
...
#define TICK_Y 0.200f
            ...
            if (!bDead && fElapsedTimeCumulated > fLocalTick) {
                ...
                bKeyRight = key_touched( flc::Key::D ) || key_touched( flc::Key::RIGHT );
                bKeyLeft  = key_touched( flc::Key::A ) || key_touched( flc::Key::LEFT  );
            ...
            auto CGE_Draw = [=]( int x, int y, char c, flc::Pixel col = flc::WHITE ) {
                DrawString( x * 8, y * 8, std::string( 1, c ), col );
            };
            auto CGE_DrawString = [=]( int x, int y, std::string s, flc::Pixel col = flc::WHITE ) {
                DrawString( x * 8, y * 8, s, col );
            };

            // Draw stats & border
            for (int i = 1; i < ScreenWidth() - 1; i++) {
                CGE_Draw( i, 0, '=', flc::DARK_YELLOW );
                CGE_Draw( i, 2, '=', flc::DARK_YELLOW );
            }
            CGE_DrawString( 5, 1, "www.OneLoneCoder.com - S N A K E ! !                SCORE: " + std::to_string( nScore ), flc::YELLOW );

			// Draw Snake Body
			for (auto s : snake)
                CGE_Draw( s.x, s.y, bDead ? '+' : '0', flc::DARK_CYAN );
			// Draw Snake Head
            CGE_Draw( snake.front().x, snake.front().y, bDead ? 'X' : '@', flc::CYAN );
			// Draw Food
            CGE_Draw( nFoodX, nFoodY, '%', flc::RED );

            if (bDead) {
                std::string sText = "PRESS <SPACE> TO PLAY AGAIN";
                CGE_DrawString( (ScreenWidth() - sText.length()) / 2, ScreenHeight() / 2, sText, flc::MAGENTA );
                if (GetKey( flc::Key::SPACE ).bPressed) {
                    bDead = false;
                    ResetGame();
                }
            }
