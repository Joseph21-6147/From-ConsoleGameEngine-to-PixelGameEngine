 ...
 *   - included <thread> to prevent error on sleep_for() call
 *   - added using namespace std::literals::chrono_literals (needed for 10ms literal)
 * Port to PixelGameEngine - 20220715
 *   - see cheat sheet for generic adaptations needed for port
 *   - no specific points
 * Port to SDL2-based Game Engine - 20221204
...
class OneLoneCoder_Maze : public flc::SDL_GameEngine
{
public:
	OneLoneCoder_Maze()
	{
...
protected:
	// Called by flcConsoleGameEngine
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
				...
				for (int py = 0; py < m_nPathWidth; py++)
					for (int px = 0; px < m_nPathWidth; px++)
					{
						if (m_maze[y * m_nMazeWidth + x] & CELL_VISITED)
							Draw(x * (m_nPathWidth + 1) + px, y * (m_nPathWidth + 1) + py, flc::WHITE); // Draw Cell
						else
							Draw(x * (m_nPathWidth + 1) + px, y * (m_nPathWidth + 1) + py, flc::BLUE); // Draw Cell
					}
		...
		for (int py = 0; py < m_nPathWidth; py++)
			for (int px = 0; px < m_nPathWidth; px++)
				Draw(m_stack.top().first * (m_nPathWidth + 1) + px, m_stack.top().second * (m_nPathWidth + 1) + py, flc::GREEN); // Draw Cell
