 ...
 *   - put the sprite file in a separate directory "WormsSprites"
 */
...
public:
	cWorm(float x = 0.0f, float y = 0.0f) : cPhysicsObject(x, y)
	{
		...
		if (sprWorm == nullptr)
			sprWorm = new olcSprite(L"WormsSprites/worms.spr");
	}
...
private:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (m_mouse[1].bReleased)
			listObjects.push_back(std::unique_ptr<cMissile>(new cMissile(m_mousePosX + fCameraPosX, m_mousePosY + fCameraPosY)));

		// Middle click to spawn worm/unit
		if (m_mouse[2].bReleased)
			listObjects.push_back(std::unique_ptr<cWorm>(new cWorm(m_mousePosX + fCameraPosX, m_mousePosY + fCameraPosY)));
