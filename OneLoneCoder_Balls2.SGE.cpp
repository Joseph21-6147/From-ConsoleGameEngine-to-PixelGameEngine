...
class CirclePhysics : public flc::SDL_GameEngine
{
public:
	CirclePhysics()
	{
...
private:
	...
	flc::Sprite *spriteBalls = nullptr;
...
public:
	...
	bool OnUserUpdate(float fElapsedTime)
	{
		...
		for (auto line : vecLines)
		{
			FillCircle(line.sx, line.sy, line.radius, flc::WHITE);
			FillCircle(line.ex, line.ey, line.radius, flc::WHITE);
		...
		// Draw Balls
		for (auto ball : vecBalls)
			FillCircle(ball.px, ball.py, ball.radius, flc::RED);

		// Draw Cue
		if (pSelectedBall != nullptr)
			DrawLine(pSelectedBall->px, pSelectedBall->py, GetMouseX(), GetMouseY(), flc::BLUE);
...
int main( int argc, char *argv[] )
{
	...
	if (game.Construct(640, 480, 2, 2))
		...
		std::wcout << "Could not construct console" << std::endl;
