...
class OneLoneCoder_PathFinding : public flc::SDL_GameEngine
{
public:
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (GetMouse(0).bReleased) // Use mouse to draw maze, shift and ctrl to place start and end
		{
			if(nSelectedNodeX >=0 && nSelectedNodeX < nMapWidth)
				...
				{
					if (GetKey(flc::Key::SHIFT).bHeld)
						nodeStart = &nodes[nSelectedNodeY * nMapWidth + nSelectedNodeX];
					else if (GetKey(flc::Key::CTRL).bHeld)
			...
			for (int y = 0; y < nMapHeight; y++)
			{
				for (auto n : nodes[y*nMapWidth + x].vecNeighbours)
				{
					DrawLine(x*nNodeSize + nNodeSize / 2, y*nNodeSize + nNodeSize / 2,
						n->x*nNodeSize + nNodeSize / 2, n->y*nNodeSize + nNodeSize / 2, flc::DARK_BLUE);
				}
			}
		...
		for (int x = 0; x < nMapWidth; x++)
			for (int y = 0; y < nMapHeight; y++)
			{
				...
				FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, nNodeSize - nNodeBorder, nNodeSize - nNodeBorder,
					nodes[y * nMapWidth + x].bObstacle ? flc::WHITE : flc::BLUE);

				if (nodes[y * nMapWidth + x].bVisited)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, nNodeSize - nNodeBorder, nNodeSize - nNodeBorder, flc::BLUE);

				if(&nodes[y * nMapWidth + x] == nodeStart)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, nNodeSize - nNodeBorder, nNodeSize - nNodeBorder, flc::GREEN);

				if(&nodes[y * nMapWidth + x] == nodeEnd)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, nNodeSize - nNodeBorder, nNodeSize - nNodeBorder, flc::RED);
		...
		if (nodeEnd != nullptr)
		{
			...
			while (p->parent != nullptr)
			{
				DrawLine(p->x*nNodeSize + nNodeSize / 2, p->y*nNodeSize + nNodeSize / 2,
					p->parent->x*nNodeSize + nNodeSize / 2, p->parent->y*nNodeSize + nNodeSize / 2, flc::YELLOW);
