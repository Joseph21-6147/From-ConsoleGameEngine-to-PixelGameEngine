...
public:
	...
	bool OnUserUpdate(float fElapsedTime)
	{
		...
		// Draw Balls
		for (auto ball : vecBalls)
			DrawWireFrameModel(this, modelCircle, ball.px, ball.py, atan2f(ball.vy, ball.vx), ball.radius, flc::WHITE);

		// Draw static collisions
		for (auto c : vecCollidingPairs)
			DrawLine(c.first->px, c.first->py, c.second->px, c.second->py, flc::RED);
