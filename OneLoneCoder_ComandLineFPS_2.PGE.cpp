...
class OneLoneCoder_UltimateFPS : public olc::PixelGameEngine
{
public:
...
protected:
	virtual bool OnUserCreate()
	{
		...
		spriteWall = new olc::Sprite("FPSSprites/fps_wall1.png");
		...
		spriteLamp = new olc::Sprite("FPSSprites/fps_lamp1.png");
		spriteFireBall = new olc::Sprite("FPSSprites/fps_fireball1.png");
	...
	virtual bool OnUserUpdate(float fElapsedTime)
	{
			...
			if (bInPlayerFOV && fDistanceFromPlayer >= 0.5f && fDistanceFromPlayer < fDepth && !object.bRemove)
			{
				...
				for (float lx = 0; lx < fObjectWidth; lx++)
				{
					for (float ly = 0; ly < fObjectHeight; ly++)
					{
						...
						olc::Pixel c = object.sprite->Sample(fSampleX, fSampleY);
						int nObjectColumn = (int)(fMiddleOfObject + lx - (fObjectWidth / 2.0f));
						if (nObjectColumn >= 0 && nObjectColumn < ScreenWidth())
							if (c != olc::BLACK && fDepthBuffer[nObjectColumn] >= fDistanceFromPlayer)
							{
		...
		for (int nx = 0; nx < nMapWidth; nx++)
			for (int ny = 0; ny < nMapWidth; ny++)
                FillRect((nx+1) * nMapScale, (ny+1) * nMapScale, nMapScale, nMapScale, map[ny * nMapWidth + nx] == '#' ? olc::WHITE : olc::VERY_DARK_GREY);

		FillRect( (1 + (int)fPlayerY) * nMapScale, (1 + (int)fPlayerX) * nMapScale, nMapScale, nMapScale, olc::YELLOW);
