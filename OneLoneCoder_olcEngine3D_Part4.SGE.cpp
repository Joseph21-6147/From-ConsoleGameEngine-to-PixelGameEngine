...
struct mesh
{
	...
	bool LoadFromObjectFile(std::string sFilename, bool bHasTexture = false)
	{
		...
		while (!f.eof())
		{
			char line[128];
			f.getline(line, 128);
			std::cout << "processing: _" << line << "_" << std::endl;
...
class flcEngine3D : public flc::SDL_GameEngine
{
public:
	flcEngine3D()
	{
...
private:
    ...
    flc::Pixel GetColour( float lum ) {

        int nValue = (int)(std::max( lum, 0.20f ) * 255.0f );
        return flc::Pixel( nValue, nValue, nValue );
    }
...
//std::cout << "a";
//		meshCube.LoadFromObjectFile( "test.obj", true );
//std::cout << "b";
		...
		sprTex1 = new flc::Sprite("Engine3DSprites/Jario.png");
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		// Clear Screen
		FillRect(0, 0, ScreenWidth(), ScreenHeight(), flc::CYAN);
		...
		for (auto &triToRaster : vecTrianglesToRaster)
		{
			...
			for (auto &t : listTriangles)
			{
				...
				DrawTriangle(t.p[0].x, t.p[0].y, t.p[1].x, t.p[1].y, t.p[2].x, t.p[2].y, flc::WHITE);
			}
		}
